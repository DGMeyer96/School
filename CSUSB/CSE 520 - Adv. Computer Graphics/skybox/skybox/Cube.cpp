...
Cube::Cube(int  &success)
{
	...
	readShaderFile((char *) "cube.vert", &vsSource);
	readShaderFile((char *) "cube.frag", &fsSource);
...
void Cube::init2DTexture()
{
	...
	for (int i = 0; i < 6; ++i) {
		...
		glTexImage2D(texAxis[i], 0, GL_RGB, texImageWidth, texImageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, texImage);
		delete texImage;
	}
