...
void init2DTexture(char *loadfile)
{
	...
	shader.readShaderFile((char *)"cubemap.vert", &VertexShaderSource);
	shader.readShaderFile((char *)"cubemap.frag", &FragmentShaderSource);
	...
	*/
	...
	//unsigned char* image = SOIL_load_image(loadfile, &width, &height, 0, SOIL_LOAD_RGB);
	nsigned char* image  = loadImageRGBA( (char *) loadfile, &width, &height);
