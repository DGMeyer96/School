...
char vs[] =  "void main(void)  \
              {              \
                 gl_Position = ftransform(); \
              }";
char fs[] = " void main(void) \
              {               \
                 gl_FragColor = vec4( 0, 1, 0, 1); \
              }";

	 
int readShaderSource( int mode, GLchar **shader )
{
    if ( mode == 0 )
      *shader = vs;
    else
...
int init(void)
{
   ...
   printf("version=%s\n", version );
   readShaderSource( 0, &VertexShaderSource );
   readShaderSource( 1, &FragmentShaderSource );
