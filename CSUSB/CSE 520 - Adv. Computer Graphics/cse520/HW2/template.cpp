...
float bSpline(int k, int m, float u, float knot[]) //k = n - 1, m = order, u = set variables, knot = standard knots
{
	float d1, d2, sum = 0.0;
...
void nurb(float control_points[][3], float u, float p[])  //what is u?
{
...
void display(void)
{
	...
	if (splineDegree > numControlPoints)#PROG	= helix-tube
#PROG	= helix-tube1
#PROG	= helix-tube2
	{
		cout << "Invalid order" << endl;#PROG	= helix-tube
#PROG	= helix-tube1
#PROG	= helix-tube2
	}
	else
	...
	*/
	...
	for(int k = 0; k < n; k++)
	{
	p[0] = bSpline(k, m, ctrlpoints0[k][0], knot); //x
	p[1] = bSpline(k, m, ctrlpoints0[k][1], knot); //y
	p[2] = bSpline(k, m, ctrlpoints0[k][2], knot); //z
