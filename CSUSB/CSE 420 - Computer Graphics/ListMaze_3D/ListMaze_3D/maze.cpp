...
void drawFloor()
{
	...
	glNormal3f(0, 0, -0.7);
	glVertex3f(0, 0, -0.7);
	glNormal3f(vertices, 0, -0.7);
	glVertex3f(vertices, 0, -0.7);
	glNormal3f(vertices, edges, -0.7);
	glVertex3f(vertices, edges, -0.7);
	glNormal3f(0, edges, -0.7);
	...
	glBegin(GL_QUADS);
	glNormal3f(0, edges, 1.5);
	glVertex3f(0, edges, 1.5);
	glNormal3f(vertices, edges, 1.5);
	glVertex3f(vertices, edges, 1.5);
	glNormal3f(vertices, 0, 1.5);
...
{
	//GLfloat position[] = { 5.0, 5.0, 0.5, 0.0 };
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'd':		//east
		tx += 1;
		break;
	case 'p':		//starts pathing animation
		//animate();
		//drawPath();
		if (specular == false)
			specular = true;
